...
class BeerMenuManager(models.Manager):
    ...
    def is_range_free(self, start, end):
        return (
            self.filter(starts_on__lte=end, ends_on__gte=start, validated=True).count()
            == 0
        )
...
class FoyerItem(models.Model):
    ...
    deposit = models.PositiveIntegerField(
        ...
        blank=True,
        default=None,
        help_text="Laisser vide si pas de caution pour cet équipement",
    )
...
class ReservationFoyer(models.Model):
    ...
    event = models.OneToOneField(
        ...
        related_name="reservation_foyer",
        null=True,
        blank=True,
    ...
    beer_selection = models.TextField(
        max_length=500,
        blank=True,
        verbose_name="Sélection des fûts",
        help_text='Indiquez sur chaque ligne "Bière : Nombre de fûts"',
    )
    items = models.ManyToManyField(
        FoyerItem, verbose_name="Équipements supplémentaires", blank=True
    )
    ...
    def send_notification(self):
        try:
            permission = Permission.objects.filter(
                ...
                codename="change_reservationfoyer",
            ).first()
