...
from cla_reservation.models import SportActivity
from cla_reservation.models._base import AbstractBlockedSlot


class ReservationDanceHallManager(models.Manager):
...
class ReservationDanceHall(models.Model):

    objects = ReservationDanceHallManager()
    ...
    event = models.OneToOneField(Event, on_delete=models.CASCADE, related_name="reservation_dancehall", null=True, blank=True, verbose_name="Réserver dans le cadre de cet événement", editable=False)
    ...
    def send_notification(self):
        try:
            permission = Permission.objects.filter(content_type__app_label='cla_reservation', codename='change_reservationdancehall').first()
            ...
            for group in groups:
                try:
                    send_mail(
                        ...
                        message="Une demande de réservation de la salle de danse a été reçue",
                        html_message=render_to_string(
                            'cla_reservation/manage/mail.html',
                            {
                                'site_href': f"https://{settings.ALLOWED_HOSTS[0]}",
                                'detail_href': f"https://{settings.ALLOWED_HOSTS[0]}{resolve_url('cla_reservation:manage:dancehall-detail', self.pk)}",
                                'reservation': self
                            }
                        ),
                    )
...
class BlockedSlotDanceHall(AbstractBlockedSlot):
