...
class AbstractEvent(models.Model):
    ...
    slug = models.SlugField(
        max_length=150, unique=True, verbose_name="Identifiant unique de l'événement"
    )
    description = SummernoteTextField(
        blank=True, verbose_name="Présentation de l'événement"
    )
    organizer = models.CharField(
        max_length=100, verbose_name="Organisateur de l'événement"
    )
    ...
    registration_starts_on = models.DateTimeField(
        verbose_name="Ouverture des inscriptions"
    )
    registration_ends_on = models.DateTimeField(
        verbose_name="Fermeture des inscriptions"
    )
    colleges = MultiSelectField(
        choices=UserInfos.Colleges.choices,
        verbose_name="Collèges autorisés à prendre une place",
        blank=True,
    )
    places = models.PositiveIntegerField(default=400, verbose_name="Nombre de places")
    contributor_ticketing_href = models.URLField(
        blank=True,
        null=True,
        verbose_name="Lien vers la billeterie d'encaissement pour les cotisants",
        help_text="Laisser vide si aucune",
    )
    non_contributor_ticketing_href = models.URLField(
        blank=True,
        null=True,
        verbose_name="Lien vers la billeterie d'encaissement pour les non cotisants",
        help_text="Laisser vide si aucune",
    )
    managers = models.ManyToManyField(
        ...
        help_text="Les administrateurs ont la possiblité de modifier les informations de l'événement ainsi que de gérer la liste des inscrits",
        blank=True,
    )
    ...
    def are_registrations_opened(self):
        return (
            self.registration_starts_on < timezone.now() <= self.registration_ends_on
            and self.places_remaining > 0
        )
...
class AbstractRegistration(models.Model):
    ...
    user = models.ForeignKey(
        ...
        help_text="Laisser vide si l'inscrit n'est pas un cotisant",
        null=True,
        blank=True,
    )
    ...
    created_by = models.ForeignKey(
        ...
        on_delete=models.SET_NULL,
        related_name="+",
        verbose_name="Créateur de cette inscription",
        editable=False,
        null=True,
    )
    ...
    mean_of_paiement = models.CharField(
        max_length=100,
        verbose_name="Moyen de paiement",
        choices=MeansOfPaiement.choices,
        null=True,
        blank=True,
    )
...
class AbstractRegistrationCustomField(models.Model):
    ...
    class Type(models.TextChoices):
        ...
        def get_inlines_types(cls):
            return {cls.TEXT, cls.SELECT, cls.CHECKBOX}
    ...
    type = models.CharField(
        max_length=255, choices=Type.choices, default=Type.TEXT, verbose_name="Type"
    )
    admin_only = models.BooleanField(
        default=False,
        verbose_name="Disponible seulement du côté administrateur",
        blank=True,
    )
    editable = models.BooleanField(
        default=False,
        verbose_name="Peut être modifié",
        help_text="Permet aux étudiants de modifier ce champ après avoir pris leur place, jusqu'au début de l'événement",
    )
    ...
    options = models.TextField(
        verbose_name="Options pour le select",
        help_text="Une valeur par ligne",
        blank=True,
        null=True,
    )
    delete_file_after_validation = models.BooleanField(
        default=False,
        verbose_name="Supprimer le fichier après validation",
        help_text="Permet de préserver la confidentialité des données une fois qu'elles ne sont plus utiles",
    )
...
class AbstractRegistrationCustomFieldValueManager(models.Manager):
    ...
    def get_or_create_text(self, registration, field, value):
        fv, created = self.get_or_create(registration=registration, field=field)
        fv.text_value = value
        fv.save()
        return fv
...
class Event(AbstractEvent):
    ...
    class Meta:
        ...
        permissions = (
            (
                "event_manager",
                "Accès à l'interface de gestion des événements pour lesquels l'utilisateur est administrateur",
            ),
        )
    ...
    allow_non_contributor_registration = models.BooleanField(
        default=False, verbose_name="Autoriser l'inscription des non cotisants"
    )
...
class EventRegistrationCustomField(AbstractRegistrationCustomField):
    event = models.ForeignKey(
        ...
        verbose_name="Champs additionnels",
    )
...
class EventRegistrationType(models.Model):
    ...
    open_to = models.CharField(
        max_length=75,
        choices=OpenTo.choices,
        verbose_name="Ouvert aux",
        default=OpenTo.BOTH,
    )
    ...
    event = models.ForeignKey(
        Event, related_name="registration_types", on_delete=models.CASCADE
    )
...
class DancingParty(AbstractEvent):
    ...
    class Meta:
        ...
        permissions = (
            (
                "dancingparty_manager",
                "Accès à l'interface de gestion des soirées dansantes pour lesquelles l'utilisateur est administrateur",
            ),
    ...
    allow_non_contributor_registration = models.BooleanField(
        default=True,
        verbose_name="Autoriser l'inscription des non cotisants par les cotisants",
    )
    scanners = models.ManyToManyField(
        User,
        related_name="+",
        verbose_name="Scanneurs",
        help_text="Les scanneurs peuvent effectuer les entrées au sein de l'événement",
        blank=True,
    )
    ...
    def checked_registrations(self):
        return (
            self.get_counted_registrations()
            .filter(checkin_datetime__isnull=False)
            .count()
        )
    ...
    def places_remaining_admin(self):
        return max(
            self.places - self.get_counted_registrations().filter(debug=False).count(),
            0,
        )
...
class DancingPartyRegistrationCustomField(AbstractRegistrationCustomField):
    dancing_party = models.ForeignKey(
        DancingParty,
        on_delete=models.CASCADE,
        related_name="custom_fields",
        verbose_name="Champs additionnels",
...
class DancingPartyRegistration(AbstractRegistration):
    ...
    staff_description = models.CharField(
        max_length=255, null=True, blank=True, verbose_name="Staff"
    )
    home = models.CharField(
        max_length=100,
        verbose_name="Logement après la soirée",
        help_text='Résidence et numéro de chambre ou bien "Lille"',
    )
    ...
    checkin_datetime = models.DateTimeField(
        null=True, verbose_name="Date et heure d'entrée"
    )
    debug = models.BooleanField(
        default=False, blank=True, verbose_name="Debug"
    )  # Special field to hide places, required `cla_ticketing.dancingparty_secretsauce` to use
    debugged_on = models.DateTimeField(
        null=True, default=None, blank=True, verbose_name="Date of debugging"
    )  # Specify the datetime of registration creation when no longer hiding
...
class DancingPartyRegistrationCustomFieldValue(AbstractRegistrationCustomFieldValue):
    registration = models.ForeignKey(
        DancingPartyRegistration,
        on_delete=models.CASCADE,
        related_name="custom_fields",
        editable=False,
    )
    field = models.ForeignKey(
        DancingPartyRegistrationCustomField,
        on_delete=models.CASCADE,
        related_name="+",
        editable=False,
    )
