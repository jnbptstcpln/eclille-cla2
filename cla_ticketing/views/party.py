...
from cla_ticketing.mixins.party import (
    DancingPartyRegistrationMixin,
    DancingPartyCollegeMixin,
)
from cla_ticketing.models import (
    DancingParty,
    DancingPartyRegistration,
    DancingPartyRegistrationCustomField,
    DancingPartyRegistrationCustomFieldValue,
)
from cla_ticketing.forms.party import (
    ContributorRegistrationForm,
    NonContributorRegistrationForm,
    RegistrationEditForm,
    RegistrationValidationForm,
)


class DancingPartyView(
    IsContributorMixin,
    DancingPartyCollegeMixin,
    DancingPartyRegistrationMixin,
    TemplateView,
...
class AbstractRegistrationCreateView(
    IsContributorMixin,
    DancingPartyCollegeMixin,
    DancingPartyRegistrationMixin,
    CreateView,
    ...
    def post(self, request, *args, **kwargs):
        if not self.party.are_registrations_opened:
            return render(
                request,
                "cla_ticketing/party/register_failed.html",
                {
                    "party": self.party,
                    "message": "La dernière place vient d'être vendue...",
                },
            )
    ...
    def form_valid(self, form):
        ...
        if self.is_contributor:
            registration.student_status = (
                DancingPartyRegistration.StudentStatus.CONTRIBUTOR
            )
            ...
            registration.student_status = (
                DancingPartyRegistration.StudentStatus.NON_CONTRIBUTOR
            )
        ...
        for custom_field in self.party.custom_fields.all():
            if custom_field.type == DancingPartyRegistrationCustomField.Type.TEXT:
                DancingPartyRegistrationCustomFieldValue.objects.get_or_create_text(
                    ...
                    field=custom_field,
                    value=form.cleaned_data[custom_field.field_id],
                )
                ...
                if form.cleaned_data[custom_field.field_id] is not None:
                    DancingPartyRegistrationCustomFieldValue.objects.get_or_create_file(
                        ...
                        value=(
                            None
                            if not form.cleaned_data[custom_field.field_id]
                            else form.cleaned_data[custom_field.field_id]
                        ),
                    )
    ...
    def get_form_kwargs(self):
        ...
        kwargs.update({"party": self.party})
        return kwargs
...
class AbstractRegistrationDetailView(
    IsContributorMixin, DancingPartyRegistrationMixin, TemplateView
):
    ...
    def get_context_data(self, **kwargs):
        ...
        context.update(
            {"registration": self.registration, "back_href": self.get_back_href()}
        )
        return context
...
class AbstractRegistrationChangeView(
    IsContributorMixin, DancingPartyRegistrationMixin, FormView
):
    registration: DancingPartyRegistration = None
    ...
    def get_back_href(self):
        if self.registration.user_id is not None:
            return resolve_url(
                "cla_ticketing:party_detail_contributor", self.party.slug
            )
        return resolve_url("cla_ticketing:party_detail_noncontributor", self.party.slug)
    ...
    def get_initial(self):
        ...
        for custom_field in self.party.custom_fields.all():
            try:
                cf_value = DancingPartyRegistrationCustomFieldValue.objects.get(
                    registration=self.registration, field=custom_field
                )
...
class ValidateRegistrationView(UserPassesTestMixin, LoginRequiredMixin, FormView):
    ...
    def get_next_registration(self, exclude_self=False):
        registrations = self.party.registrations.filter(
            Q(validated=False) | Q(paid=False), is_staff=False, debug=False
        )
        if self.registration:
            registrations = registrations.filter(
                created_on__gte=self.registration.created_on
            )
            if exclude_self:
    ...
    def test_func(self):
        ...
        return (
            self.registration
            and self.registration.dancing_party.managers.filter(
    ...
    def get(self, request, *args, **kwargs):
        ...
        if self.request.GET.get("registration_pk"):
            self.registration = self.party.registrations.filter(
                Q(validated=False) | Q(paid=False),
                pk=self.request.GET.get("registration_pk"),
            ).first()
    ...
    def post(self, request, *args, **kwargs):
        self.registration = self.party.registrations.filter(
            pk=self.request.POST.get("registration_pk")
        ).first()
        if self.registration is None:
            messages.warning(
                request,
                "L'inscription à laquelle vous souhaitiez accéder n'existe plus",
            )
    ...
    def form_valid(self, form):
        ...
        if self.registration.validated:
            # Clear files
            for cs in self.registration.custom_fields.filter(
                field__type=DancingPartyRegistrationCustomField.Type.FILE
            ):
    ...
    def get_inline_custom_fields(self):
        return (
            [
                (cs, self.registration.custom_fields.filter(field=cs).first())
                for cs in self.party.custom_fields.filter(
                    type__in=DancingPartyRegistrationCustomField.Type.get_inlines_types()
                )
            ]
            if self.registration
            else []
    ...
    def get_block_custom_fields(self):
        return (
            [
                (cs, self.registration.custom_fields.filter(field=cs).first())
                for cs in self.party.custom_fields.filter(
                    type__in=DancingPartyRegistrationCustomField.Type.get_block_types()
                )
            ]
            if self.registration
            else []
    ...
    def get_initial(self):
        return (
            {"paid": self.registration.paid, "validated": self.registration.validated}
            if self.registration
            else {}
        )
...
class CheckInPartyView(UserPassesTestMixin, LoginRequiredMixin, TemplateView):
    ...
    def post(self, request: HttpRequest, *args, **kwargs):
            ...
            results = [
                {
                    ...
                    "href": resolve_url(
                        "cla_ticketing:party_checkin_registration",
                        self.party.slug,
                        r.pk,
                    ),
                }
                for r in self.party.registrations.annotate(
        ...
        return JsonResponse({"success": True, "payload": results})
    ...
    def get_context_data(self, **kwargs):
        ...
        context.update(
            {
                "party": self.party,
                "is_manager": self.request.user.has_perm(
                    "cla_ticketing.dancingparty_manager"
                )
                or self.party.managers.filter(pk=self.request.user.pk).count() > 0,
            }
        )
        return context
...
class CheckInQRCodeView(UserPassesTestMixin, LoginRequiredMixin, View):
    ...
    def get(self, request: HttpRequest, *args, **kwargs):
        try:
            payload = jwt.decode(
                jwt=kwargs.pop("token"), key=settings.SECRET_KEY, algorithms=["HS256"]
            )
        ...
        try:
            r = DancingPartyRegistration.objects.get(
                pk=payload.get("pk", None), dancing_party=self.party
            )
            return JsonResponse(
                {
                    "success": True,
            ...
            return JsonResponse({"success": False, "message": "No matched"}, status=404)
...
class CheckInRegistrationView(UserPassesTestMixin, LoginRequiredMixin, TemplateView):
    ...
    def setup(self, request, *args, **kwargs):
        self.registration = get_object_or_404(
            DancingPartyRegistration,
            pk=kwargs.pop("registration_pk"),
            dancing_party__slug=kwargs.pop("party_slug"),
        )
        super().setup(request, *args, **kwargs)
    ...
    def test_func(self):
        if self.request.user.has_perm("cla_ticketing.dancingparty_manager"):
            ...
            self.registration.dancing_party.managers.filter(
                pk=self.request.user.pk
            ).count()
            > 0
        ):
            return True
        return (
            self.registration.dancing_party.scanners.filter(
                pk=self.request.user.pk
            ).count()
            > 0
        )
    ...
    def post(self, request: HttpRequest, *args, **kwargs):
        ...
        return redirect(
            "cla_ticketing:party_checkin", self.registration.dancing_party.slug
        )
        ...
        context.update(
            {
                ...
                or self.registration.dancing_party.managers.filter(
                    pk=self.request.user.pk
                ).count()
                > 0,
                "registration": self.registration,
